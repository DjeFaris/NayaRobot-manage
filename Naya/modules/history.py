...
async def cek_mataa(self: Client, ctx: Message, strings):
    ...
    if usernamebefore != ctx.from_user.username:
        usernamebefore = f"@{usernamebefore}" if usernamebefore else strings("<code>Tanpa Username</code>")
        usernameafter = f"@{ctx.from_user.username}" if ctx.from_user.username else strings("<code>Tanpa Username</code>")
